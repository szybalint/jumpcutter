...
def splitVideoToSmallParts(INPUT_FILE):
    ...
    outputpath = os.path.join("parts", filename + "_shortened%03d.mp4")
    command = "ffmpeg -i " + INPUT_FILE + " -crf 22 -map 0 -segment_time " + str(segment_time) + \
...
def concatenateVideoParts(outputfile):
    ...
    result_parts = os.listdir("result")
    partnames_concatenated = "("
    for result_part in result_parts:
        partnames_concatenated += "echo file '" + os.path.join("result", result_part) + "' &"
...
parser.add_argument('--silent_threshold', type=float, default=0.01,
...
if args.url != None:
    ...
    input_files = splitVideoToSmallParts(INPUT_FILE)
else:
    INPUT_FILE = args.input_file
    input_files = splitVideoToSmallParts(INPUT_FILE)
FINAL_OUTPUT_FILE = inputToOutputFilename(INPUT_FILE)

createPath("result")
currentOutput = 0
for file in input_files:
    INPUT_FILE = os.path.join("parts", file)
    URL = args.url
    ...
    if len(args.output_file) >= 1:
        ...
        OUTPUT_FILE = inputToOutputFilename(INPUT_FILE, str(currentOutput))
...
concatenateVideoParts(FINAL_OUTPUT_FILE)
